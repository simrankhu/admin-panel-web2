...
if(isset($_GET['type']) && $_GET['type']!=''){
    ...
    if($type=='status'){
        ...
        $update_status_sql="update technical set status='$status' where id='$id'";
        mysqli_query($conn,$update_status_sql);
         ?>
        <script>
            window.location.href='technical.php';
        </script>
        <?php
    }
    ...
    if($type=='delete'){
        ...
        $delete_sql="delete from technical where id='$id'";
...
/*add data */
  ...
  if(isset($_GET['id']) && $_GET['id']!=''){
    ...
    $res=mysqli_query($conn,"select * from technical where id='$id'");
 ...
 if(isset($_POST['submit'])){
  ...
  $icon =str_replace("---","-",preg_replace("/[^-a-zA-Z0-9s]/", "-", strtolower(trim($name))));
...
if($msg==''){
        ...
        if(isset($_GET['id']) && $_GET['id']!=''){
            $id=$_GET['id'];
               ...
               {
                    mysqli_query($conn,"update  `technical` SET `icon`='$icon', `name`='$name',`description`='$description' WHERE id='$id'");
               }else{
                    mysqli_query($conn,"update  `technical` SET `icon`='$icon', `name`='$name',`description`='$description',`image`='$filename' WHERE id='$id'");
               }
                 echo "<script>alert('Updated successfully')</script>";  
                   ...
                   if((empty($_FILES['uploadfile']['name'])))
                     {
                          mysqli_query($conn, "INSERT INTO `technical`(`icon`, `name`, `description`,`status`) VALUES ('$icon','$name','$description','1')"); 
                     }else{
                    mysqli_query($conn, "INSERT INTO `technical`(`icon`, `name`, `description`,`status`,`image`) VALUES ('$icon','$name','$description','1','$filename')"); 
                     }
              ...
              <table id="example1" class="table table-bordered table-striped">
               ...
               ?>
                ...
                <td><?php echo $i ?></td>    
                  ...
                  <td> <img src="../admin/uploads/service/<?php echo $row['image'] ?>" style="height: 80px;"></img></td>
                  ...
                  <td>
                      <ul class="edit-button">
                          <li><a href="technical.php?id=<?php echo $row['id'] ?>"><i class="fa fa-edit"></i></a></li>
                          <li>
                            <?php 
