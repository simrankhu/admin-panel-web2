...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM tbl_certificates WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE tbl_certificates SET title='".$title."',status='".$status."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO tbl_certificates SET title='".$title."',status='".$status."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
			    ...
			    $galleryimagename = "";
					...
					$queryinfo = "SELECT image FROM `tbl_certificates` WHERE id = '".$id."'";
					...
					$galleryimagename =  trim($resultinfo['image']);
						...
						if($galleryimagename!="")
						{
						$unlkheaderfile = "uploads/certificate/".$galleryimagename;
						if (file_exists($unlkheaderfile)) { unlink($unlkheaderfile); }
						}
					$galleryfilename = "uploads/certificate/". $galleryimage;
					$mv =move_uploaded_file($_FILES['image']['tmp_name'],$galleryfilename);
					$query_imageup="UPDATE tbl_certificates SET image='".$galleryimage."' WHERE id='".$id."'";
					if($sql_imageup=$conn->prepare($query_imageup))
					$sql_imageup->execute();
			} 

			
			echo "<script>document.location.href='view-certificate.php?msg=".$sess_msg."';</script>";
			exit;
		}
	}
}
...
if($id!="")
{
    $query_select="SELECT * FROM tbl_certificates WHERE id='".$id."'";
	...
	{ 
		...
		{
			echo "<script>document.location.href='view-certificate.php';</script>";
			exit;
		}
    }
}
              ...
              <div class="row">
               <div class="col-md-4">
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="form-group">
					<?php if($image!="") { ?>
					<img src="uploads/certificate/<?php echo $result['image']?>" height="100" width="120" title="<?php echo $title; ?> Image">
					<?php } ?>
