...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM vgallaery WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE vgallaery SET title='".$title."',url='".$url."',status='".$status."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO vgallaery SET title='".$title."',url='".$url."',status='".$status."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
			    ...
			    $galleryimagename = "";
					if($id!="")
					{
					$queryinfo = "SELECT image FROM `vgallaery` WHERE id = '".$id."'";
					...
					$query_imageup="UPDATE vgallaery SET image='".$galleryimage."' WHERE id='".$id."'";
					if($sql_imageup=$conn->prepare($query_imageup))
					$sql_imageup->execute();
			} 
			echo "<script>document.location.href='view-vgallery.php?msg=".$sess_msg."';</script>";
			exit;
		}
	}
}
...
if($id!="")
{
    $query_select="SELECT * FROM vgallaery WHERE id='".$id."'";
	...
	{ 
		...
		{
			echo "<script>document.location.href='view-vgallery.php';</script>";
			exit;
		}
    }
}
          ...
          <div class="field-section">
              ...
              <div class="row">
               <div class="col-md-4">
                  ...
                  <div class="form-group">
                      <input type="text" name="url" value="<?php echo $url; ?>" class="from-control custom-type">
                   </div>
                  </div>
     <!--             <div class="col-md-4">-->
