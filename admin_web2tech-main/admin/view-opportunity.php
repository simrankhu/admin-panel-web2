...
if(isset($_GET['id']) && $_GET['id']!="")
{
	...
	$query_delete="DELETE FROM  tbl_opportunity WHERE id='".$id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{
			$query_select= "SELECT * from  tbl_opportunity WHERE id ='".$value."'";
			if($sql_select=$conn->query($query_select))
			{
			...
			if($sql_deleteall=$conn->query("DELETE FROM  tbl_opportunity WHERE id = '".$value."'"))
			{
				$sess_msg="Deleted Successfully.";					
			}
	    }
...
if(isset($_POST['chk_btn']) && !empty($_POST['chk_btn']))
{
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		$query_act="UPDATE tbl_opportunity SET status = '1' WHERE id IN (".$str_rest_refs.")";
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
           ...
           <h3>View Opportunity</h3>
		   <?php } ?>
           </div>
		   <form  action="<?php $_SERVER['PHP_SELF'] ?>" method ="post">
           <a href="add-opportunity.php" class="btn btn-addnew">Add New</a>
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php   $query="SELECT * FROM tbl_opportunity ORDER BY id DESC";
                 ...
                 <td>
                  ...
                  <td>
                      ...
                      <ul class="edit-button">
                          <li><a href="add-opportunity.php?id=<?php echo $row['id'];?>"><i class="fa fa-edit"></i></a></li>
...
<script>
  ...
  swal({   title: "Are you sure?", 
    text: "Want remove this Opportunity", 
    type: "warning",  
    ...
    function(isConfirm){   
      if (isConfirm) {    
       $.ajax({
        ...
        data: {'id': ids ,'remove_opportunity':'action'} ,
        type: "POST",
        success: function (data) {
          if(data=='OK'){
            swal("Remove!", "Opportunity has been removed", "success"); 
            location.reload();
          }
          else{
