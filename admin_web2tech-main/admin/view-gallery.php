...
{
	$id = $_GET['id'];
	$query_select = "SELECT * FROM gallery WHERE id ='".$id."'";
	if($sql_select = $conn->query($query_select))
	{
		$result = $sql_select->fetch_array(MYSQLI_ASSOC);
		@unlink("uploads/gallery/".$result['image']);
	}
	$query_delete="DELETE FROM gallery WHERE id='".$id."'";
	if($sql_delete=$conn->query($query_delete))
	{
		$sess_msg="Deleted Successfully.";
		echo "<script>document.location.href='view-gallery.php?msg=".$sess_msg."';</script>";
	    exit();					
	}
}
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{
			$query_select= "SELECT * from gallery WHERE id ='".$value."'";
			if($sql_select=$conn->query($query_select))
			{
			...
			if($sql_deleteall=$conn->query("DELETE FROM gallery WHERE id = '".$value."'"))
			{
				$sess_msg="Deleted Successfully.";					
			}
	    }
...
if(isset($_POST['chk_btn']) && !empty($_POST['chk_btn']))
{
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		$query_act="UPDATE gallery SET status = '1' WHERE id IN (".$str_rest_refs.")";
...
if(isset($_POST['unchk_btn']) && !empty($_POST['unchk_btn']))
{
	...
	{
		$str_rest_refs=implode(",",$_POST['check_status']);
		$query_deact="UPDATE gallery SET status = '0' WHERE id IN (".$str_rest_refs.")";
...
if(isset($_POST['galleryathome']) && !empty($_POST['galleryathome']))
{
	...
	{
		$str_rest_refs=implode(",",$_POST['check_status']);
		$query_deact="UPDATE gallery SET category = 'home' WHERE id IN (".$str_rest_refs.")";
		if($sql_deact=$conn->prepare($query_deact)) 
		{
			$sql_deact->execute();
			$sess_msg="Deactivated Successfully.";	
...
if(isset($_POST['regalleryathome']) && !empty($_POST['regalleryathome']))
{
	...
	{
		$str_rest_refs=implode(",",$_POST['check_status']);
		$query_deact="UPDATE gallery SET category = 'gallery' WHERE id IN (".$str_rest_refs.")";
		if($sql_deact=$conn->prepare($query_deact)) 
		{
			$sql_deact->execute();
			$sess_msg="Deactivated Successfully.";	
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
           ...
           <a href="add-gallery.php" class="btn btn-addnew">Add New</a>
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                <thead>
                  ...
                  <input type="submit" name="regalleryathome" value="Remove from Home" onClick="return confirm('Are you sure? You want to Remove from Home');" class="btn btn-warning">
                   ...
                   <input type="submit" class="btn btn-danger" style="margin-top:2px;" name="delete_all" value="Delete" onClick="return confirm('Are you sure? You want to Delete');"></td>
                ...
                <tbody>
                  <?php   $query="SELECT * FROM gallery ORDER BY id DESC";
                ...
                <td><?php echo $i; ?></td>    
                  ...
                  <td><img src="uploads/gallery/<?php echo $row['image']; ?>" width="70px" height="70px" ></td>
                 ...
                 <td>
                  ...
                  <td>
                      ...
                      <ul class="edit-button">
                           <li><a href="add-gallery.php?id=<?php echo $row['id'];?>"><i class="fa fa-edit"></i></a></li>
    ...
    function(isConfirm){   
      if (isConfirm) {    
       $.ajax({
        ...
        data: {'id': ids ,'remove_gallery':'action'} ,
        type: "POST",
