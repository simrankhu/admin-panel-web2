...
if(isset($_GET['id']) && $_GET['id']!="")
{
	...
	if($sql_select = $conn->query($query_select))
	{
		...
		@unlink("uploads/news/".$result['image']);
	}
	$query_delete="DELETE FROM tbl_news WHERE id='".$id."'";
...
{ 
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		{
			$query_select= "SELECT * from tbl_news WHERE id ='".$value."'";
			if($sql_select=$conn->query($query_select))
			{
			...
			if($sql_deleteall=$conn->query("DELETE FROM tbl_news WHERE id = '".$value."'"))
			{
				$sess_msg="Deleted Successfully.";					
			}
	    }
...
if(isset($_POST['chk_btn']) && !empty($_POST['chk_btn']))
{
	if(isset($_POST['check_status']) && count($_POST['check_status'])>0)
	{
		...
		$query_act="UPDATE tbl_news SET status = '1' WHERE id IN (".$str_rest_refs.")";
...
if(isset($_POST['unchk_btn']) && !empty($_POST['unchk_btn']))
{
	...
	{
		$str_rest_refs=implode(",",$_POST['check_status']);
		$query_deact="UPDATE tbl_news SET status = '0' WHERE id IN (".$str_rest_refs.")";
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
           ...
           <a href="add-newsevent.php" class="btn btn-addnew">Add New</a>
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                <?php   $query="SELECT * FROM tbl_news ORDER BY id DESC";
                ...
                <td><?php echo $i; ?></td>    
                  ...
                  <td><img src="uploads/news/<?php echo $row['image']; ?>" width="70px" height="70px" ></td>
                    <td>Yes</td>
                 ...
                 <td>
                  ...
                  <td>
                      ...
                      <ul class="edit-button">
                          <li><a href="add-newsevent.php?id=<?php echo $row['id'];?>"><i class="fa fa-edit"></i></a></li>
...
<script>
  ...
  swal({   title: "Are you sure?", 
    text: "Want remove this News Event", 
    type: "warning",  
    ...
    function(isConfirm){   
      if (isConfirm) {    
       $.ajax({
        ...
        data: {'id': ids ,'remove_news':'action'} ,
        type: "POST",
        success: function (data) {
          if(data=='OK'){
            swal("Remove!", "News Event has been removed", "success"); 
            location.reload();
          }
          else{
