...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM  tbl_live WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE  tbl_live SET title='".$title."',status='".$status."',content='".$content."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO  tbl_live SET title='".$title."',status='".$status."',content='".$content."'";
...
if($id!="")
{
	...
	{ 
		...
		{
			echo "<script>document.location.href='view-live.php';</script>";
			exit;
		}
    }
}
      ...
      <div class="row">
	   ...
	   <?php if($msg!="") { ?><h5 class="h5-headn" style="color:red;"><?php echo $msg; ?></h5><?php } else { ?>
          ...
          <div class="field-section">
              ...
              <div class="row">
               <div class="col-md-4">
                    <label class="input-text">Live Webinars Title</label>
