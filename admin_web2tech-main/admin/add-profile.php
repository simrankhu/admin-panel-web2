...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM sucess_story WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE sucess_story SET title='".$title."',status='".$status."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO sucess_story SET title='".$title."',status='".$status."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
			    ...
			    $galleryimagename = "";
					...
					$queryinfo = "SELECT image FROM `sucess_story` WHERE id = '".$id."'";
					...
					$query_imageup="UPDATE sucess_story SET image='".$galleryimage."' WHERE id='".$id."'";
					if($sql_imageup=$conn->prepare($query_imageup))
					$sql_imageup->execute();
			} 

			
			echo "<script>document.location.href='view-profile.php?msg=".$sess_msg."';</script>";
			exit;
		}
	}
}
...
if($id!="")
{
    $query_select="SELECT * FROM sucess_story WHERE id='".$id."'";
	...
	{ 
		...
		{
			echo "<script>document.location.href='view-profile.php';</script>";
			exit;
		}
    }
}
