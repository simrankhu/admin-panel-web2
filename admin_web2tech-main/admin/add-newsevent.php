...
if(isset($_POST['submit']) && $_POST['submit']=="add")
{
	...
	$query_duplicate="SELECT * FROM tbl_news WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		else
		{
			if($id!="")
			{
				$query_update="UPDATE tbl_news SET title='".$title."',status='".$status."',line1='".$line1."',line2='".$line2."',content='".$content."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO tbl_news SET title='".$title."',status='".$status."',line1='".$line1."',line2='".$line2."',content='".$content."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
				...
				$tbl_newsimage = $array[0];
				$tbl_newsimage1 = $array[1];
				$time =time();
				$tbl_newsimage = $time.$tbl_newsimage;
				$tbl_newsimage = str_replace("---","-",preg_replace("/[^-a-zA-Z0-9s]/", "-", strtolower(trim($tbl_newsimage)))); 
				$tbl_newsimage = $tbl_newsimage.".".$tbl_newsimage1;  

			    $tbl_newsimagename = "";
					if($id!="")
					{

					$queryinfo = "SELECT image FROM `tbl_news` WHERE id = '".$id."'";
					...
					$tbl_newsimagename =  trim($resultinfo['image']);
					}
					}
					}
						if($tbl_newsimagename!="")
						{
						$unlkheaderfile = "uploads/news/".$tbl_newsimagename;
						if (file_exists($unlkheaderfile)) { unlink($unlkheaderfile); }
						}
					$tbl_newsfilename = "uploads/news/". $tbl_newsimage;
					$mv =move_uploaded_file($_FILES['image']['tmp_name'],$tbl_newsfilename);
					$query_imageup="UPDATE tbl_news SET image='".$tbl_newsimage."' WHERE id='".$id."'";
					if($sql_imageup=$conn->prepare($query_imageup))
					$sql_imageup->execute();
			} 

			echo "<script>document.location.href='view-newsevent.php?msg=".$sess_msg."';</script>";
			exit;
		}
	}
}
...
if($id!="")
{
    $query_select="SELECT * FROM tbl_news WHERE id='".$id."'";
	...
	{ 
		...
		{
			echo "<script>document.location.href='view-newsevent.php';</script>";
			exit;
		}
    }
}
      ...
      <div class="row">
	   ...
	   <?php if($msg!="") { ?><h5 class="h5-headn" style="color:red;"><?php echo $msg; ?></h5><?php } else { ?>
          ...
          <div class="field-section">
              ...
              <div class="row">
               <div class="col-md-4">
                    <label class="input-text">News Event Title</label>
                  ...
                  <div class="col-md-4">
                    ...
                    <div class="form-group">
					<?php if($image!="") { ?>
					<img src="uploads/news/<?php echo $result['image']?>" height="100" width="120" title="<?php echo $title; ?> Image">
					<?php } ?>
