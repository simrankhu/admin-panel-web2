...
if(isset($_GET['type']) && $_GET['type']!=''){
    ...
    if($type=='status'){
        ...
        $update_status_sql="update why1 set status='$status' where id='$id'";
        mysqli_query($conn,$update_status_sql);
         ?>
        <script>
            window.location.href='why1.php';
        </script>
        <?php
    }
    ...
    if($type=='delete'){
        ...
        $delete_sql="delete from why1 where id='$id'";

        mysqli_query($conn,$delete_sql);
        echo '<script>alert("why1 Deleted Succesfully")</script>';
        ?>
           <script>
            window.location.href = 'why1.php';
           </script>
...
/*add data */
  ...
  if(isset($_GET['id']) && $_GET['id']!=''){
    ...
    $res=mysqli_query($conn,"select * from why1 where id='$id'");
...
if($msg==''){
        ...
        if(isset($_GET['id']) && $_GET['id']!=''){
            $id=$_GET['id'];
               ...
               {
                    mysqli_query($conn,"update  `why1` SET `icon`='$icon', `name`='$name',`description`='$description' WHERE id='$id'");
               }else{
                    mysqli_query($conn,"update  `why1` SET `icon`='$icon', `name`='$name',`description`='$description',`image`='$filename' WHERE id='$id'");
               }
                 echo "<script>alert('Updated successfully')</script>";  
                   ...
                   if((empty($_FILES['uploadfile']['name'])))
                     {
                          mysqli_query($conn, "INSERT INTO `why1`(`icon`, `name`, `description`,`status`) VALUES ('$icon','$name','$description','1')"); 
                     }else{
                    mysqli_query($conn, "INSERT INTO `why1`(`icon`, `name`, `description`,`status`,`image`) VALUES ('$icon','$name','$description','1','$filename')"); 
                     }
           ...
           <h3>View Why Choose Us</h3>
            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               $sql=mysqli_query($conn,"select * from why1");
               $i=1;
                ...
                <td><?php echo $row['description'] ?></td>
                  ...
                  <td>
                      <ul class="edit-button">
                          <li><a href="why1.php?id=<?php echo $row['id'] ?>"><i class="fa fa-edit"></i></a></li>
                          <li>
                            <?php 
