...
{
	$category = addslashes(ucwords(trim($_POST['category'])));
	...
	$query_duplicate="SELECT * FROM gallery WHERE title='".$title."' $checkDuplicate";
	if($sql_duplicate=$conn->query($query_duplicate))
	{
		...
		{ */
			if($id!="")
			{
				$query_update="UPDATE gallery SET title='".$title."',status='".$status."',category='".$category."',content='".$content."' WHERE id='".$id."'";
			...
			{
				$query_insert="INSERT INTO gallery SET title='".$title."',status='".$status."',category='".$category."',content='".$content."'";
			...
			if(isset($_FILES['image']) && $_FILES['image']['error']==0)
			{
			    ...
			    $galleryimagename = "";
					...
					$queryinfo = "SELECT image FROM `gallery` WHERE id = '".$id."'";
					...
					$query_imageup="UPDATE gallery SET image='".$galleryimage."' WHERE id='".$id."'";
...
if($id!="")
{
	...
	{ 
		...
		{
			echo "<script>document.location.href='gallery_mgmt.php';</script>";
			exit;
		}
    }
}
              ...
              <div class="row">
               <div class="col-md-6">
                  ...
                  <div class="form-group">
                     <input type="text" name="category" value="<?php echo $category; ?>" class="from-control custom-type">
                   </div>
                  </div>
                   <div class="col-md-6">
				...
				<div id="status_status"></div>
                  ...
                  <div class="col-md-12">
                    <label class="input-text">Short Content</label>
                  <div class="form-group">
                      <textarea name="content" class="from-control custom-type" rows="10" cols="5"><?php echo $content;?></textarea>
                   </div>
                  </div>
                  <div class="col-md-12">
